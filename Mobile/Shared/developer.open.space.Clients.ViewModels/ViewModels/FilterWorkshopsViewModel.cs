...
{
    public class FilterWorkshopsViewModel : ViewModelBase
    {
        private IUnityContainer _container;
        private developer.open.space.DataStore.Abstractions.DataObjects.Category _allCategory;
        private developer.open.space.DataStore.Abstractions.DataObjects.Category _showPastCategory;
        private developer.open.space.DataStore.Abstractions.DataObjects.Category _showFavoritesCategory;

        public FilterWorkshopsViewModel(INavigationService navigationService, IEventAggregator eventAggregator, IStoreManager storeManager, IToast toast, IFavoriteService favoriteService, ILoggerFacade logger, ILaunchTwitter twitter, ISSOClient ssoClient, IPushNotifications pushNotifications, IReminderService reminderService, IPageDialogService pageDialogService, IUnityContainer container)
            : base(navigationService, eventAggregator, storeManager, toast, favoriteService, logger, twitter, ssoClient, pushNotifications, reminderService, pageDialogService)
        {
            ...
            _allCategory = new developer.open.space.DataStore.Abstractions.DataObjects.Category
            {
                Name = "All",
                IsEnabled = true,
        ...
        private void SetShowAllCategories(bool showAll)
        {
            ...
            {
                var category = (categoryCell?.View?.BindingContext as CategoryCellViewViewModel)?.Category as developer.open.space.DataStore.Abstractions.DataObjects.Category;
                if (category != null)
                {
        ...
        public ICommand DoneCommand => DelegateCommand.FromAsyncHandler(async () => 
        {
            ...
            Settings.ShowPastWorkshops = _showPastCategory.IsFiltered;
            Save();
        ...
        public async Task LoadCategoriesAsync()
        {
            ...
            _showPastCategory.IsFiltered = Settings.ShowPastWorkshops;
        ...
        {
            Filters.Clear();
            _showPastCategory = new developer.open.space.DataStore.Abstractions.DataObjects.Category
            {
                Name = "Show Past Workshops",
                IsEnabled = true,
                ShortName = "Show Past Workshops",
            ...
            var showFavoritesCategory = new developer.open.space.DataStore.Abstractions.DataObjects.Category
            {
        ...
        public void Save()
        {
            Settings.FilteredCategories = string.Join("|", Categories?.Where(c => ((developer.open.space.DataStore.Abstractions.DataObjects.Category)(c?.View?.BindingContext as CategoryCellViewViewModel)?.Category).IsFiltered).Select(c => ((developer.open.space.DataStore.Abstractions.DataObjects.Category)(c?.View?.BindingContext as CategoryCellViewViewModel)?.Category).Name));
        }
    }
}
